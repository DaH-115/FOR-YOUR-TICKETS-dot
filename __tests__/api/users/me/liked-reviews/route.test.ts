import { GET } from "@/api/users/me/liked-reviews/route";
...
describe("GET /api/users/me/liked-reviews", () => {
  ...
  describe("인증 검증", () => {
    test("인증되지 않은 요청은 401 에러를 반환한다", async () => {
      ...
      const req = new NextRequest("http://localhost/api/users/me/liked-reviews");
  ...
  describe("성공 케이스", () => {
    test("인증된 사용자의 좋아요 리뷰 목록을 반환한다", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/users/me/liked-reviews?page=1&pageSize=10",
      );
      const res = await GET(req);
      const body = await res.json();
    ...
    test("fetchLikedReviewsPaginated에 올바른 파라미터를 전달한다", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/users/me/liked-reviews?page=2&pageSize=20",
      );
      await GET(req);
  ...
  describe("페이지네이션", () => {
    ...
    test("페이지 파라미터를 정수로 파싱한다", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/users/me/liked-reviews?page=3&pageSize=15",
      );
      await GET(req);
    ...
    test("잘못된 페이지 파라미터는 NaN으로 처리된다", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/users/me/liked-reviews?page=invalid",
      );
      await GET(req);
  ...
  describe("검색 기능", () => {
    ...
    test("검색어를 올바르게 전달한다", async () => {
      ...
      const req = new NextRequest(
        `http://localhost/api/users/me/liked-reviews?search=${encodeURIComponent(searchTerm)}`,
      );
      await GET(req);
  ...
  describe("통합 시나리오", () => {
    test("인증 + 페이지네이션 + 검색이 모두 함께 동작한다", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/users/me/liked-reviews?page=2&pageSize=10&search=영화",
      );
      const res = await GET(req);
      const body = await res.json();
